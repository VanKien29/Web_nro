...
class CostumeService extends AdminServiceSupport
{
    ...
    public function get(int $id): array
    {
        ...
        if (!$item || (int) ($item->TYPE ?? -1) !== 5) {
            return ['ok' => false, 'status' => 404, 'message' => 'Cải trang không tồn tại'];
        }
    ...
    public function create(Request $request): array
    {
        ...
        try {
            ...
            ] = $this->storeCostumeSpriteUploads($request);

            $itemIconId = $this->storeCostumeItemIcon($request, $idMap, $iconIds, $saved);
            $avatarId = $this->storeCostumeAvatarIcon($request, $idMap, $iconIds, $saved);
            if ($avatarId === null && $iconIds) {
                $avatarId = (int) array_key_first($iconIds);
            }
            if ($avatarId === null || $avatarId <= 0) {
                throw new \InvalidArgumentException('Cần upload ít nhất một icon hoặc avatar x4.');
            }
            $itemIconId ??= $avatarId;
            ...
            return ['ok' => false, 'status' => 422, 'message' => 'Dữ liệu cải trang không hợp lệ: ' . $e->getMessage()];
        }
        ...
        try {
            ...
            foreach ($partRows as $row) {
                ...
                $createdParts[] = $row['id'];
                if ((int) $row['TYPE'] === 0) {
                    $game->table('head_avatar')->updateOrInsert(['head_id' => $row['id']], ['avatar_id' => $avatarId]);
                }
            }
            ...
            return ['ok' => false, 'status' => 500, 'message' => 'Không tạo được cải trang: ' . $e->getMessage()];
        }

        $this->logAdminAction('costume.create', 'item_template', $itemId, 'Tạo cải trang ' . $name, null, [
            'item' => $itemRow,
    ...
    public function update(Request $request, int $id): array
    {
        ...
        $saved = [];
        $oldPartIds = $this->partIdsFromCostumeItem($item);
        ...
        try {
            ...
            $itemIconId = $this->storeCostumeItemIcon($request, $idMap, $iconIds, $saved) ?? $itemIconId;
            $avatarId = $this->storeCostumeAvatarIcon($request, $idMap, $iconIds, $saved) ?? $avatarId;
            if ($avatarId <= 0) {
                $avatarId = $itemIconId;
            }
        ...
        try {
            ...
            foreach (array_merge([$headId], $extraIds) as $partId) {
                $game->table('head_avatar')->updateOrInsert(['head_id' => $partId], ['avatar_id' => $avatarId]);
            }
            ...
            return ['ok' => false, 'status' => 500, 'message' => 'Không sửa được cải trang: ' . $e->getMessage()];
        }

        $this->logAdminAction('costume.update', 'item_template', $id, 'Sửa cải trang ' . $name, $this->sanitizeLogState((array) $item), [
            'item' => $itemRow,
    ...
    public function delete(int $id): array
    {
        ...
        $partIds = array_fill_keys($this->partIdsFromCostumeItem($item), true);
        $avatarIdFromComment = $this->costumeAvatarIdFromComment((string) ($item->comment ?? ''));
        if ($avatarIdFromComment > 0) {
            foreach ($game->table('head_avatar')->where('avatar_id', $avatarIdFromComment)->pluck('head_id') as $headId) {
                $headId = (int) $headId;
        ...
        if ($deletedParts) {
            foreach ($game->table('part')->whereIn('id', $deletedParts)->get(['DATA']) as $partRow) {
                foreach ($this->assets->decodePartData($partRow->DATA ?? '') as $layer) {
                    $iconIdsToDelete[] = (int) ($layer['icon_id'] ?? 0);
                }
            }
            foreach ($game->table('head_avatar')->whereIn('head_id', $deletedParts)->pluck('avatar_id') as $avatarId) {
        ...
        try {
            ...
            return ['ok' => false, 'status' => 500, 'message' => 'Không xóa được cải trang: ' . $e->getMessage()];
        }

        $this->logAdminAction('costume.delete', 'item_template', $id, 'Xóa cải trang ' . ($item->NAME ?? ('#' . $id)), $this->sanitizeLogState((array) $item), [
    ...
    private function storeCostumeSpriteUploads(Request $request): array
    {
        $saved = [];
        $idMap = [];
        $iconIds = [];
    ...
    private function storeCostumeItemIcon(Request $request, array &$idMap, array &$iconIds, array &$saved): ?int
    {
    ...
    private function storeCostumeAvatarIcon(Request $request, array &$idMap, array &$iconIds, array &$saved): ?int
    {
        $avatarFile = $this->assets->requestFiles($request, 'avatar_x4')[0] ?? null;
        if (!$avatarFile) {
            return null;
        }

        $candidate = $this->assets->numericIdFromFilename($avatarFile->getClientOriginalName());
        $avatarId = $this->assets->resolveGameAssetId($candidate, 'icon', $iconIds);
        $saved = array_merge($saved, $this->assets->saveGamePngPyramid($avatarFile, 'data/icon', "{$avatarId}.png"));
        if ($candidate !== null) {
    ...
    private function partIdsFromCostumeItem(object $item): array
    {
    ...
    private function looksLikeCostumeExtraHead($game, int $headId, array $knownPartIds): bool
    {
        ...
        return !$game->table('item_template')
