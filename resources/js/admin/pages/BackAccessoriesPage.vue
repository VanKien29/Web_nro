    ...
    <div class="back-accessories-page">
        ...
        <div class="card">
                    ...
                    <tbody>
                        <tr v-for="item in rows" :key="item.id">
                            ...
                            <td>
                                ...
                                <div class="item-meta">
                                    icon {{ item.icon_id }} · type {{ item.type }}
                                </div>
                            </td>
                            <td>
                            ...
                            <td class="icon-data-cell">{{ item.icon_data }}</td>
            ...
            <div v-if="totalPages > 1" class="pagination">
                <button :disabled="page <= 1" @click="load(page - 1)">&laquo;</button>
                <button
        ...
        <div v-if="editor.open" class="modal-overlay" @click.self="closeEditor">
            <div class="modal-panel">
                <div class="modal-head">
                    <div>
                        <h3>{{ editor.form.id ? "Sửa đeo lưng" : "Thêm đeo lưng" }}</h3>
                        <p>Ảnh sprite chỉ cần x4, hệ thống tự tạo x3/x2/x1 và đổi ID nếu trùng.</p>
                    </div>
                        ...
                        <div class="preview-card">
                            <div class="preview-icon-wrap">
                                <img
                                    ...
                                    alt="Icon preview"
                                />
                                <span v-else class="mi preview-empty">backpack</span>
                            </div>
                            <strong>{{ editor.form.name || "Đeo lưng mới" }}</strong>
                            <small>part = flag_bag.id</small>
                        </div>
                    </aside>
                    ...
                    <div class="editor-main">
                        <section class="editor-card">
                            <div class="form-grid">
                                ...
                                <label>
                                    <span class="form-label">FlagBag ID muốn chèn</span>
                                    <input
                                ...
                                <label>
                                    <span class="form-label">Gold flag_bag</span>
                                    <input
                            ...
                            <div class="upload-grid">
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <small>{{ fileSummary("icon_x4") || "Kéo thả nhiều ảnh/folder" }}</small>
                                    </label>
                                </div>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <small>{{ fileSummary("item_icon_x4") || "Ảnh item_template.icon_id" }}</small>
                                    </label>
                                </div>
                            </div>
                        </section>

                        <section v-if="editor.form.icon_data" class="editor-card">
                            <label>
                                <span class="form-label">Icon data hiện tại</span>
                                <textarea
                ...
                <div class="modal-actions">
                    ...
                    <button class="btn btn-primary" :disabled="editor.saving" @click="saveBackAccessory">
    ...
    methods: {
        ...
        async load(page = this.page) {
            ...
            try {
                ...
                const res = await fetch(`/admin/api/back-accessories?${params}`, {
                    headers: {
        ...
        async openEditor(item = null) {
            ...
            try {
                const res = await fetch(`/admin/api/back-accessories/${item.id}`, {
                    headers: {
                        Accept: "application/json",
                        "X-Requested-With": "XMLHttpRequest",
                    },
        ...
        setFiles(field, fileList) {
            ...
            this.editor.files[field] = field === "item_icon_x4" ? files.slice(0, 1) : files;
            this.dragField = "";
            if (field === "item_icon_x4" || (!this.previewIcon && field === "icon_x4")) {
                this.revokePreview();
        ...
        saveDraft() {
            ...
            try {
                localStorage.setItem(this.draftKey(), JSON.stringify({ form: this.editor.form }));
                this.editor.error = "";
        ...
        applyDraft() {
            ...
            try {
                ...
                this.editor.error = "";
                this.success = "Đã nạp bộ đeo lưng đã lưu. File ảnh cần chọn lại.";
            } catch {
        ...
        async saveBackAccessory() {
            ...
            if (!this.editor.form.id && !(this.editor.files.icon_x4 || []).length) {
            ...
            try {
                const spriteFiles = this.sortedSpriteFiles(this.editor.files.icon_x4 || []);
                if (spriteFiles.length) {
                    formData.set("icon_x4_payload", JSON.stringify(await this.filesToPayload(spriteFiles)));
                }
        ...
        async readJsonResponse(res) {
            ...
            try {
                ...
                const plain = text
                    ...
                    .replace(/\s+/g, " ")
                    .trim();
                throw new Error(plain || `API trả về dữ liệu không phải JSON (${res.status})`);
            }
        },
    },
};
