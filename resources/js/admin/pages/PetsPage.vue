...
export default {
    ...
    methods: {
        ...
        draftKey() {
            return "admin_pet_create_draft_v1";
        },
        loadDraftEditor() {
            try {
        ...
        saveDraft() {
            ...
            try {
                ...
                this.editor.error = "";
                this.success = "Đã lưu bộ Pet đang nhập.";
            } catch {
        ...
        applyDraft() {
            ...
            this.editor.error = "";
            this.success = "Đã nạp bộ Pet đã lưu. File ảnh cần chọn lại.";
        },
        token() {
            return document
        ...
        async load(page = this.page) {
            ...
            try {
                ...
                const res = await fetch(`/admin/api/pets?${params}`, {
                    headers: {
                ...
                if (!res.ok || !data.ok) {
                    throw new Error(data.message || "Không tải được Pet");
                }
                this.rows = data.data || [];
                this.page = data.page || page;
                this.totalPages = data.total_pages || 1;
            } catch (error) {
                this.error = error?.message || "Không tải được Pet";
            } finally {
                this.loading = false;
            }
        },
        ...
        async openEditor(item = null) {
            ...
            try {
                const res = await fetch(`/admin/api/pets/${item.id}`, {
                    headers: {
                        Accept: "application/json",
                        "X-Requested-With": "XMLHttpRequest",
                    },
                ...
                this.editor.error = error?.message || "Không tải được Pet";
            } finally {
        ...
        async savePet() {
            if (!this.editor.form.name) {
                this.editor.error = "Tên Pet không được để trống.";
                return;
            }
            if (
                !this.editor.form.head_data ||
            ...
            try {
                ...
                const res = await fetch(
                    isEdit
                        ? `/admin/api/pets/${this.editor.form.id}`
                        : "/admin/api/pets",
                    {
                    method: "POST",
                    headers: {
                ...
                const data = await this.readJsonResponse(res);
                if (!res.ok || !data.ok) {
                    throw new Error(data.message || "Không lưu được Pet");
                }
                this.success = data.message || "Đã lưu Pet";
                if (!isEdit) {
                    this.clearDraft();
                }
                this.closeEditor();
                ...
                this.editor.error =
                    error?.message || "Không lưu được Pet";
            } finally {
                this.editor.saving = false;
            }
        },
        async deletePet(item) {
            ...
            try {
                ...
                const data = await this.readJsonResponse(res);
                if (!res.ok || !data.ok) {
                    throw new Error(data.message || "Không xóa được Pet");
                }
                this.success = data.message || "Đã xóa Pet";
                await this.load(this.page);
            } catch (error) {
                this.error = error?.message || "Không xóa được Pet";
            } finally {
