...
<div class="card" style="max-width:720px;">
    <form method="POST" action="{{ route('admin.giftcodes.update', $giftcode->id) }}" id="giftcodeForm">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <input type="hidden" name="detail" id="detailInput" value="{{ old('detail', $giftcode->detail ?? '[]') }}">
        </div>
        ...
        <div class="flex gap-2">
            <button class="btn btn-primary" type="submit">
                <span class="material-icons-round" style="font-size:16px;">save</span> Lưu thay đổi
            </button>
            <a href="{{ route('admin.giftcodes.index') }}" class="btn btn-outline">Quay lại</a>
        </div>
    </form>

    <hr style="border-color:rgba(255,255,255,.06); margin:28px 0;">
    <form method="POST" action="{{ route('admin.giftcodes.destroy', $giftcode->id) }}"
        onsubmit="return confirm('Xoá giftcode này?')">
        @csrf
        @method('DELETE')
        <button class="btn btn-danger btn-sm" type="submit">
            <span class="material-icons-round" style="font-size:14px;">delete</span> Xoá giftcode
        </button>
    </form>
</div>
@endsection
...
<script>
    ...
    const itemMapData = @json($itemMap);
    const iconBase = '/assets/frontend/home/v1/images/x4/';
    let items = [];

    // Build item map for resolving names
    const itemMap = {};
    Object.keys(itemMapData).forEach(function(k) {
        itemMap[k] = itemMapData[k];
    });

    // Load existing detail
    try {
        const existing = JSON.parse(document.getElementById('detailInput').value);
        if (Array.isArray(existing)) {
            existing.forEach(function(item) {
                const info = itemMap[item.temp_id];
                items.push({
                    temp_id: item.temp_id,
                    name: info ? info.name : 'Item #' + item.temp_id,
                    icon_id: info ? info.icon_id : item.temp_id,
